...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `*Oiee @${num.split('@')[0]}\ seja bem vindo(a) ao grupo ${mdata.subject}* *Siga as regras e evite ser banido(a) do grupo, espero que faça muitas amizades por aqui💜😙*`
				...
				teks = `*Sayonara @${num.split('@')[0]} Espero que você possa voltar*`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				...
				success: '️*❬ ✔ ❭ PRONTINHO*',
				error: {
					stick: 'Essa opção não está funcionando!',
				...
				only: {
					...
					admin: ' *❬ ⚠️ ❭ COMANDO PERMITIDO SOMENTE PARA ADMINISTRADOR ❬ ⚠️ ❭* ',
...
case 'lofi':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '️*Amo lofi*'})
					break
                 case 'logoph':
					var gh = body.slice(9)
					var gbl1 = gh.split("|")[0];
                ...
                case 'reportarbug':
                    ...
                    reply('OBRIGADO COLEGA, O BUG FOI REPORTADO COM SUCESSO✅')
                    break
					case 'marcar':
					members_id = []
			...
			case 'mapa': 
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`Algo deu errado ;(, Desculpe-me`)
							})
							.on('end', function () {
								console.log('Finish')
								buff = fs.readFileSync(ran)
						...
						reply(`*Esse comando ${prefix} é para fazer figurinhas*`)
					}
					break
				case 'gtts':	
				case 'tts':
				case 'voz':
					if (args.length < 1) return client.sendMessage(from, '*Escolher um Idioma é nescessário!*', text, {quoted: mek})
					const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return client.sendMessage(from, '*Qual mensagem deseja que eu reproduza?*', text, {quoted: mek})
					dtt = body.slice(9)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 300
					? reply('*Esse texto é muito grande😤*')
				...
				case 'tiktokperfil':
					try {
						if (args.length < 1) return client.sendMessage(from, '*Qual o nome da pessoa?*', text, {quoted: mek})
				...
				case 'limpar':
			       ...
			       case 'bloquear':
					...
					client.sendMessage(from, `*Membro bloqueado com sucesso✅* ${body.slice(7)}@c.us`, text)
					break
                    case 'desbloquear':
					if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply(mess.only.ownerB)
				...
				case 'adicionar':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('*Qual o número da pessoa que você deseja adicionar ao grupo?* ')
					...
					try {
						...
						console.log('Error :', e)
						reply(' *Falha ao adicionar, talvez seja por quê o número esteja privado* ')
					}
					break
					case 'grup':
					case 'group':
					...
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args[0] === 'abrir') {
					    reply(` *Grupo aberto, agora todos os membros podem enviar mensagem*`)
						client.groupSettingChange(from, GroupSettingChange.messageSend, false)
					} else if (args[0] === 'fechar') {
						reply(` *Grupo fechado, somente administradores podem enviar mensagem* `)
       ...
       client.sendMessage(from, ' *Este é o número do meu criador^^* ',MessageType.text, { quoted: mek} )
           ...
           case 'rebaixar':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Marque a pessoa que deseja remover o administrador!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += ` *Um dos administradores removeu o seu administrador, sinto muito 😕* :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					} else {
						mentions(` *Um dos administradores removeu o seu administrador @${mentioned[0].split('@')[0]} sinto muito por isso😕* `, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
				case 'promote':
				case 'promover':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('*Marque a pessoa que você deseja tornar administrador!*')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += ` *Parabéns😙 você agora é o mais novo Hokage do grupo!* :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
